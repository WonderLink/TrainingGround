...
def hIndex(indexSet):#求一个点的h0
    Indexs=sorted(indexSet)
    for index in Indexs:
        clist = [i for i in indexSet if i >= index]
        if index >= len(clist):
            break
    return index
...
def calcHIndexValues(nxG,igG,n):
	...
	'''
	if n == 1:
		return hIndex(neighborDegree)
	else:
		return CalcuHIndex(neighborDegree, n - 1)
	# result = []
	# return result
